@php
    $heroSettings = app(\App\Services\WorkProgramService::class)->getSettings();
@endphp
<x-layouts.app>
    <div class="relative min-h-screen">
        
        <!-- Section 1: Hero Program Kerja (Full Screen) -->
        <div class="relative w-full h-screen flex items-center overflow-hidden">
            ...
            <div class="absolute inset-0 z-0">
                ...
                @endphp
                <img src="{{ $bgUrl ?: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&q=80&w=1600' }}" alt="Program Kerja Background" class="w-full h-full object-cover object-[center_30%]">
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-20">
                <div class="max-w-2xl space-y-6 animate-fade-in-up">
                    <h1 class="text-3xl sm:text-4xl lg:text-[42px] font-extrabold text-white tracking-tight leading-tight">
                        {{ $heroSettings['hero_title_1'] ?? 'Program Kerja' }} <span class="text-orange-500">{{ $heroSettings['hero_title_2'] ?? 'Unggulan' }}</span>
                    </h1>
                    
                    <p class="text-slate-200 text-[13px] sm:text-sm leading-relaxed max-w-xl font-medium">
                        {{ $heroSettings['hero_desc'] ?? 'PIK-R REQUEST SMAN 1 Tasik Putri Puyu menyelenggarakan sistem edukasi dan konseling remaja melalui berbagai macam program kerja & aksi nyata. Saat ini, program kerja kami diimplementasikan melalui metode Peer-to-Peer Learning yang bermitra langsung dengan pihak sekolah dan instansi terkait.' }}
                    </p>

                    <div class="flex flex-wrap gap-3 pt-2">
                        <a href="#daftar-proker" class="px-6 py-2.5 bg-teal-400/90 text-white rounded-xl font-bold text-[12px] hover:bg-teal-500 transition-all shadow-lg flex items-center gap-2">
                            Apa itu Program Kerja? <i class="fas fa-arrow-right text-[10px]"></i>
                        </a>
                        <a href="#" class="px-6 py-2.5 border-2 border-white text-white rounded-xl font-bold text-[12px] hover:bg-white hover:text-[#1e3a5f] transition-all flex items-center gap-2">
                            Lihat Dokumentasi <i class="fas fa-arrow-right text-[10px]"></i>
                        </a>
                    </div>
                </div>
            </div>
        ...
        <!-- Section 2: Program Kerja Utama (Alternating Layout) -->
        <div id="daftar-proker" class="max-w-7xl mx-auto px-4 py-20 sm:py-32 space-y-24 sm:space-y-40">
            <div class="text-center mb-20 sm:mb-32">
                <h2 class="text-2xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
                    Program Kerja <span class="text-orange-500">Utama</span>
                </h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-20 items-center {{ $index % 2 != 0 ? 'lg:flex-row-reverse' : '' }}">
                ...
                <div class="relative {{ $index % 2 != 0 ? 'lg:order-1' : 'lg:order-2' }}">
                    <div class="relative rounded-3xl overflow-hidden shadow-2xl">
                        <img src="{{ $item->image ? asset('storage/' . $item->image) : 'https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?auto=format&fit=crop&q=80&w=1200' }}" alt="{{ $item->title }}" class="w-full h-auto object-cover aspect-[4/3] hover:scale-105 transition-transform duration-700">
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-900/20 to-transparent"></div>
                    </div>
                    <!-- Decorative Element -->
                    <div class="absolute -bottom-6 -right-6 w-32 h-32 bg-orange-500/10 rounded-full blur-2xl -z-10"></div>
                </div>
            </div>
            @empty
            <div class="text-center py-20">
                <p class="text-slate-500">Belum ada program kerja yang ditambahkan.</p>
            </div>
            @endforelse
        </div>

<x-layouts.dashboard title="Tambah Layanan Konseling | Admin PIK-R">
    <div>
        <header class="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-black text-slate-800">Tambah Konseling</h1>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Tambah data konseling baru</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-10 max-w-4xl mx-auto w-full">
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <form action="{{ route('dashboard.counselings.store') }}" method="POST" class="p-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required placeholder="Contoh: Kecemasan Ujian Nasional">
                                @error('title')
                            ...
                            <div>
                                ...
                                <select name="topic" id="topic" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required>
                                    ...
                                    <option value="Kesehatan Mental" @if(old('topic') == 'Kesehatan Mental') selected @endif>Kesehatan Mental</option>
                                    <option value="Akademik & Belajar" @if(old('topic') == 'Akademik & Belajar') selected @endif>Akademik & Belajar</option>
                                    <option value="Hubungan Sosial" @if(old('topic') == 'Hubungan Sosial') selected @endif>Hubungan Sosial</option>
                                    <option value="Keluarga" @if(old('topic') == 'Keluarga') selected @endif>Keluarga</option>
                                    <option value="Karir & Masa Depan" @if(old('topic') == 'Karir & Masa Depan') selected @endif>Karir & Masa Depan</option>
                                    <option value="Lainnya" @if(old('topic') == 'Lainnya') selected @endif>Lainnya</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select name="student_class" id="student_class" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all">
                                    <option value="" selected>Tidak Disebutkan</option>
                                    <option value="Kelas X" @if(old('student_class') == 'Kelas X') selected @endif>Kelas X</option>
                                    <option value="Kelas XI" @if(old('student_class') == 'Kelas XI') selected @endif>Kelas XI</option>
                                    <option value="Kelas XII" @if(old('student_class') == 'Kelas XII') selected @endif>Kelas XII</option>
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" name="counselor_name" id="counselor_name" value="{{ old('counselor_name') }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" placeholder="Contoh: Budi Santoso">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all">
                                @error('date')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required>
                                    ...
                                    <option value="Sedang Berjalan" @if(old('status') == 'Sedang Berjalan') selected @endif>Sedang Berjalan</option>
                                    <option value="Menunggu" @if(old('status') == 'Menunggu') selected @endif>Menunggu</option>
                                    <option value="Dibatalkan" @if(old('status') == 'Dibatalkan') selected @endif>Dibatalkan</option>
                                </select>
                                @error('status')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" placeholder="Tuliskan catatan konseling...">{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>

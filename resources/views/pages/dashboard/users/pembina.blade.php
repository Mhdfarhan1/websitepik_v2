<x-layouts.dashboard title="Akun Pembina | Admin PIK-R">
    <div x-data="{ showModal: false }">
        <header class="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
            <div>
                <h1 class="text-xl font-black text-slate-800">Akun Pembina</h1>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Kelola Akses Untuk Pembina PIK-R</p>
            </div>
            <x-dashboard.button @click="$dispatch('open-modal', { name: 'add-pembina' })" variant="primary" icon="fas fa-plus">
                Tambah Pembina
            </x-dashboard.button>
        </header>
                ...
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-black text-xs border border-blue-100 shadow-sm group-hover:scale-110 transition-transform">
                    ...
                    <td class="px-8 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            <x-dashboard.button 
                                @click="$dispatch('open-modal', { name: 'edit-pembina', data: { id: {{ $user->id }}, name: '{{ $user->name }}', email: '{{ $user->email }}' } })"
                            ...
                            <form action="{{ route('dashboard.users.destroy', $user) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center shadow-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus akun pembina ini?')">
                ...
                <tr>
                    <td colspan="4" class="px-8 py-10 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-sm font-bold text-slate-400">Belum ada akun pembina</p>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        <!-- Add User Modal -->
        <x-dashboard.modal name="add-pembina" title="Tambah Pembina Baru" :show="$errors->any() && !old('id')">
            <form action="{{ route('dashboard.users.store') }}" method="POST" class="space-y-6">
                @csrf
                <input type="hidden" name="role" value="pembina">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" name="name" required class="w-full bg-slate-50 border-2 border-transparent focus:border-blue-500/10 focus:bg-white rounded-2xl px-12 py-4 text-sm font-bold text-slate-600 transition-all shadow-sm" placeholder="Contoh: Budi Santoso, S.Pd">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="email" name="email" required class="w-full bg-slate-50 border-2 border-transparent focus:border-blue-500/10 focus:bg-white rounded-2xl px-12 py-4 text-sm font-bold text-slate-600 transition-all shadow-sm" placeholder="nama@instansi.com">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="password" name="password" required class="w-full bg-slate-50 border-2 border-transparent focus:border-blue-500/10 focus:bg-white rounded-2xl px-12 py-4 text-sm font-bold text-slate-600 transition-all shadow-sm" placeholder="Masukkan password kuat...">
                    </div>
                </div>

                <div class="pt-6 flex gap-3">
                    <x-dashboard.button @click="$dispatch('close-modal', { name: 'add-pembina' })" variant="secondary" class="flex-1 !rounded-2xl py-4">Kembali</x-dashboard.button>
        ...
        <!-- Edit User Modal -->
        <x-dashboard.modal name="edit-pembina" title="Edit Akun Pembina" :show="$errors->any() && old('id')">
            <div x-data="{ id: '{{ old('id') }}', name: '{{ old('name') }}', email: '{{ old('email') }}' }" 
                 @open-modal.window="if($event.detail.name === 'edit-pembina') { id = $event.detail.data.id; name = $event.detail.data.name; email = $event.detail.data.email; }">
                <form :action="`{{ url('dashboard/users') }}/${id}`" method="POST" class="space-y-6">
                    ...
                    <div class="pt-6 flex gap-3">
                        <x-dashboard.button @click="$dispatch('close-modal', { name: 'edit-pembina' })" variant="secondary" class="flex-1 !rounded-2xl py-4">Kembali</x-dashboard.button>

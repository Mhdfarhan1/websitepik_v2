<x-layouts.dashboard title="Edit Edukasi Sebaya | Admin PIK-R">
    <div>
        <header class="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-black text-slate-800">Edit Edukasi Sebaya</h1>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Ubah data Edukasi Sebaya</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-10 max-w-4xl mx-auto w-full">
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <form action="{{ route('dashboard.peer-educations.update', $peerEducation) }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-8">
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="title" id="title" value="{{ old('title', $peerEducation->title) }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required>
                                @error('title')
                            ...
                            <div>
                                ...
                                <textarea name="desc" id="desc" rows="4" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required>{{ old('desc', $peerEducation->desc) }}</textarea>
                                @error('desc')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="desc2" id="desc2" rows="4" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all">{{ old('desc2', $peerEducation->desc2) }}</textarea>
                                @error('desc2')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="participant_count" id="participant_count" value="{{ old('participant_count', $peerEducation->participant_count) }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required min="0">
                            ...
                            <div>
                                ...
                                <input type="number" name="order_index" id="order_index" value="{{ old('order_index', $peerEducation->order_index) }}" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all">
                                @error('order_index')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-2xl hover:border-[#1e3a8a] hover:bg-slate-50 transition-all group relative overflow-hidden">
                                    ...
                                    @if($peerEducation->image)
                                        <img id="preview" src="{{ asset('storage/' . $peerEducation->image) }}" class="absolute inset-0 w-full h-full object-cover rounded-2xl opacity-40 group-hover:opacity-20 transition-opacity">
                                    @else

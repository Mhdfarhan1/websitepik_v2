<x-layouts.dashboard title="Tambah Laporan | Admin PIK-R">
    <div>
        <header class="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-black text-slate-800">Tambah Laporan</h1>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Buat Laporan baru</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-10 max-w-4xl mx-auto w-full">
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                <form action="{{ route('dashboard.reports.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    ...
                    <div class="space-y-8">
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="desc" id="desc" rows="4" class="w-full px-4 py-3 rounded-xl bg-slate-50 border-transparent focus:border-[#1e3a8a] focus:bg-white focus:ring-0 text-sm font-medium transition-all" required placeholder="Tuliskan deskripsi utama Laporan ini...">{{ old('desc') }}</textarea>
                                @error('desc')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Maks. 10MB</p>
                                @error('document')
                                    <p class="text-xs text-rose-500 mt-2 font-medium">{{ $message }}</p>
                                @enderror
                            </div>

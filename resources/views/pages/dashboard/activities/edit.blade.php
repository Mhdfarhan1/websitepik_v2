<x-layouts.dashboard title="Edit Agenda Kegiatan | Admin PIK-R">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
            <form action="{{ route('dashboard.activities.update', $activity) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1.5">
                    ...
                    @if($activity->image)
                        <div class="mb-2 w-32 h-20 rounded-xl overflow-hidden border border-slate-200">
                    ...
                    <p class="text-[10px] text-slate-400">Pilih file baru jika ingin mengganti poster lama (Format: JPG, PNG, WEBP. Maksimal 10MB).</p>
                </div>
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-8 py-3 bg-[#1e40af] hover:bg-blue-800 text-white font-bold text-xs rounded-2xl shadow-lg shadow-blue-900/10 transition-all active:scale-95 flex items-center gap-2">
                        ...
                        <span>Update Data Kegiatan</span>
                    </button>
                </div>
            </form>
        </div>
